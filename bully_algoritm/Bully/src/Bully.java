...
public class Bully implements Runnable {
     ...
     * sender, timer and heartbeat thread. The timer thread waits few seconds to
     * receive a response. If it receives an OK but doesn't receive a leader
...
//////////////////////////////////////////////////////////////////
 ...
 /**
 * The sendOK() method sends OK message to the incoming process
which has requested an election request.
 *
 */
 ...
 */
 public static void sendElectionRequest() throws IOException {
        ...
        // fiecare nod stie de existenta celorlalte noduri
        // cum se completeaza aceasta structura de date ?
        for (int key : processes.keySet()) {
            // System.out.print("\nprocess " + key);
            if (key > self_id) {
            //System.out.println(" higher");
                String destination_server = processes.get(key);
                try {
                    if (destination_server.equals("cti.ubm.ro")) {
                        //destination_server="193.231.17.125";
                    }
                    if (destination_server.equals("s3.ubm.ro")) {
                        //destination_server="193.231.17.67";
                    }
                    if (destination_server.equals("node01")) {
                        destination_server = "192.168.3.162";
                    }
                    if (destination_server.equals("raspberrypi")) {
                        destination_server = "192.168.3.67";
                    }
                    //System.out.println("\n#0 destination_server:" +destination_server);
                    Socket socket = new Socket(destination_server, server_Port);
                    //System.out.println("\n#1 connected on port:" + server_Port);
                    DataOutputStream out = new DataOutputStream(socket.getOutputStream());
     ...
     *
     */
    public static void initialize(String myname, String myid) throws UnknownHostException, IOException {
        InetAddress iAddress;
        String server_name = "nicht";
        processes.put(2, "cti.ubm.ro");
        processes.put(5, "s3.ubm.ro");
        if (myname != null && !myname.isEmpty()) {
            ...
            server_name = iAddress.getHostName();
            if (server_name.equals("s3.ubm.ro")) {
                self_id = 5;
                higher = 0;
            } else if (server_name.equals("cti.ubm.ro")) {
                self_id = 2;
                higher = 1;
            } else if (server_name.equals("raspberrypi")) {
                self_id = 1;
                higher = 1;
            }
        }
        System.out.println("\nthis node is " + server_name + " (" + self_id + ")");
// Election initiator
        Runnable sender = new Bully("sender", "elecreq");
        new Thread(sender).start();
    }
